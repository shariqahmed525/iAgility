...
const WhyChooseBox = ({ name, coloredText, text, theme }) => {
  return (
    ...
    >
      <CgCheckO
        className={`my-2 ml-1 self-start text-4xl sm:text-2xl text-${theme}`}
      />
      <p className="text-base xs:text-lg px-3 sm:px-0 font-bold w-full sm:text-left">
        {name} <span className={`text-${theme}`}> {coloredText}</span>
      </p>
      <p className="hidden sm:inline-block text-xs font-light py-3">
        <q>{text}</q>
      </p>
      <BsFillPlusCircleFill
        className={`inline sm:hidden my-2 ml-1 text-4xl text-theme cursor-pointer`}
      />
    </div>
  );
};
